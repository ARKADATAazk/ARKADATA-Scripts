...
-- >>>>>>>>>>>>>>>>>>>>>>> UI CORE GUARDS (injected) <<<<<<<<<<<<<<<<<<<<<<
-- Prevent 'table index is nil' and nil 'target' during early UI build

els = els or {}

do
  local _adopt = adoptChild
  function adoptChild(parent, child)
    child  = child  or {}
    parent = parent or {}
    -- Ensure targets exist for widgets that assume them
    child.target  = child.target  or child
    parent.target = parent.target or parent
    -- Ensure parent containers/defaults
    parent.children = parent.children or {}
    parent.block    = parent.block    or 1
    parent.z        = parent.z        or 1
    if type(_adopt) == 'function' then
      local ok = pcall(_adopt, parent, child)
      if not ok then
...
do
  ...
  function AddEl(o)
    o = o or {}
    -- Inherit block/z from parent or default to 1 so els[o.block][o.z] is always addressable
    o.block = o.block or (o.parent and o.parent.block) or 1
    o.z     = o.z     or (o.parent and o.parent.z)     or 1
    els[o.block] = els[o.block] or {}
    if type(_AddEl) == 'function' then
      return _AddEl(o)
    else
      if o.parent and adoptChild then pcall(adoptChild, o.parent, o) end
      if not els[o.block][o.z] then els[o.block][o.z] = {o} else table.insert(els[o.block][o.z], o) end
      return o
    end
  end
end
...
showPages = {Global=true, TCP=true, EnvCP=true, MCP=true, Transport=true, Generic_Global=false, Generic=true, Errors=false, Default6=true} 
...
function El:arrange()
  ...
  -- ===== Nested page-aware visibility (Default6 + D6_* subpages) =====
  if self.belongsToPage then
    local tag = self.belongsToPage
    local visible = true

    if tag == 'Default6' then
      visible = (activePage == 'Default6')
    elseif type(tag) == 'string' and tag:match('^D6_') then
      visible = (activePage == 'Default6' and _G and _G.d6SubTag == tag)
    else
      visible = (activePage == tag)
    end

    if not visible then
      -- remember original interactivity only once, then disable
      if self._wasInteractive == nil then
        self._wasInteractive = (self.interactive ~= false)
      end
      self.hidden = true
      ...
      if self.hidden == true then
        self.hidden = nil
        -- restore original interactive state if we had disabled it
        if self._wasInteractive ~= nil then
          self.interactive = self._wasInteractive
          self._wasInteractive = nil
        end
        if addNeedUpdate then addNeedUpdate(self, true) end
      end
    end
  end
...
function Populate()
  ...
  pageMenuButton:new({parent=pageMenu, buttonImg='page_global', textStr='DEFAULT 6.0', value='Default6'})
...
---
---
local EXT_NS = 'reARK_TA'
local function GetHideV6() return reaper.GetExtState(EXT_NS,'hideV6') == '1' end
local function SetHideV6(b) reaper.SetExtState(EXT_NS,'hideV6', b and '1' or '0', true) end

-- Compact full-width header row for Generic (keeps the grey bar)
local function GenericHeaderRow(parent, text, opts)
  opts = opts or {}
  local HDR_H  = opts.h      or 20   -- bar height
  local GAP_T  = opts.mt     or 2    -- smaller top gap
  local GAP_B  = opts.mb     or 1    -- smaller bottom gap
  local BARCOL = opts.barCol or c_Grey50
  local TXTCOL = opts.txtCol or c_Grey10

  local hdr = El:new({
    parent = parent, tile = true, x = 0, y = 0, w = 0, h = HDR_H + GAP_T + GAP_B,
    onArrange = function(k)
      local cw = (k.parent.drawW and k.parent.drawW/scaleMult) or k.parent.w or 800
      k.w = math.max(120, cw - 40)  -- span all columns (container minus 2×20px gutters)
    end
  })

  El:new({
    parent = hdr, x = 0, y = GAP_T, w = 0, r = {toEdge, hdr, 'right'},
    h = HDR_H, col = BARCOL,
    text = { str = text, style = 2, align = 4, col = TXTCOL }
  })
  return hdr
end
...
if reaper.ThemeLayout_GetParameter(0) == nil then
  noParamsBox = El:new({
    parent=bodyBox, x=0, y=0, w=500, h=60, border=boxBorder, col=c_Grey10,
    text={str='', style=3, align=5, col=c_Grey50},
    onArrange=function(k)
  ...
  -- Title row (separate so headers never overlap it)
  local titleRow = El:new({
    parent=bodyBox, tile=true, x=0, y=0, w=0, h=6,
    onArrange=function(k)
      local cw = (k.parent.drawW and k.parent.drawW/scaleMult) or k.parent.w or 800
      k.w = math.max(120, cw - 40)
    end
  })
  El:new({
    parent=titleRow, x=6, y=2, z=2, w=0, h=20, r={toEdge, titleRow, 'right'},
    interactive=false,
    text={str='', style=4, align=4, col=c_Grey70},
    onArrange=function(k) k.text.str = themeTitle end
  })

-- after your left title element in titleRow:
local v6Ctrl = Controller:new({ parent=titleRow })
v6Ctrl.paramV = GetHideV6() and 1 or 0

local rightBox = El:new({
  parent=titleRow, x=0, y=2, w=320, h=22,
  onArrange=function(k)
    local pw = k.parent.w or 800
    k.x = math.max(0, pw - 320 - 8) -- stick right with 8px gutter
  end
})

local function toggleV6()
  local newState = (v6Ctrl.paramV == 1) and 0 or 1
  v6Ctrl.paramV = newState
  SetHideV6(newState == 1)
  if requestArrange then requestArrange() end   -- or your own “queue redraw”
end

Button:new({
  parent=rightBox, x=0, y=0, w=28, h=22, style='button',
  target=v6Ctrl, img='button_off', imgOn='button_on', iType=3,
  onClick=function() toggleV6() end
})

Label:new({
  parent=rightBox, x=36, y=2, w=280, h=18, target=v6Ctrl,
  text={ str='Hide Internal V6 Params (Default 6.0)', style=2, align=0,
         resizeToFit=true, col=labelColMA, mouseOverCol=labelColMO },
  onClick=function() toggleV6() end
})

  local i=0
  local firstGenericHeader = true
  local COL_W  = 340   -- column width
  local ROW_H  = 6    -- compact row height that still fits 20px knob
  local PAD_Y  = 1     -- small vertical padding

  while reaper.ThemeLayout_GetParameter(i) ~= nil do
    local _, description = reaper.ThemeLayout_GetParameter(i)
    if description then
      local entryController = Controller:new({
        parent=bodyBox, x=0, y=0, w=0, h=0, desc=description, param=i
      })

      -- SECTION HEADER (min=max=0) ➜ full-width grey bar spanning all columns
      if entryController.paramVMin == 0 and entryController.paramVMax == 0 then
        GenericHeaderRow(bodyBox, entryController.desc, {
          mt = firstGenericHeader and 6 or 2,  -- tighter than before
          mb = -16,
          h  = 25,
          barCol = c_Grey50
        })
        firstGenericHeader = false

        
      else
        -- NORMAL PARAM ROW ➜ fixed-width tile so it packs into columns
        local entry = El:new({ parent=bodyBox, tile=true, x=0, y=0, w=COL_W, h=ROW_H })

        if entryController.paramVMin == 0 and entryController.paramVMax == 1 then
          -- boolean toggle (original placement)
          Button:new({
            parent=entry, x=0, y=PAD_Y, flow=true, style='button',
            target=entryController, img='button_off', imgOn='button_on', iType=3
          })
        else
          -- restore original look: 20px knob with grey circle
          El:new({ parent=entry, x=2, y=PAD_Y, w=20, h=20, z=1, shape='evenCircle', col=c_Grey50, interactive=false })
          Knob:new({
            parent=entry, target=entryController, x=2, y=PAD_Y,
            w=20, h=20, img='knobStack_20px_dark', iFrameH=20, iFrame=20
          })
        end

        -- label (original size/placement)
        Label:new({
          parent=entry, flow=true, x=6, y=PAD_Y, z=2, w=60, h=20, target=entryController,
          text={ str=entryController.desc, style=2, align=0, padding=0,
                 resizeToFit=true, col=c_Grey70, mouseOverCol=c_Grey80 }
        })

        -- numeric controls for non-boolean params
        if not (entryController.paramVMin == 0 and entryController.paramVMax == 1) then
          SpinnerH:new({ parent=entry, flow=true, x=4, y=PAD_Y-1, z=2, w=36, h=20, target=entryController })
          Readout:new({
            parent=entry, flow=true, x=6, y=PAD_Y, z=2, w=56, h=20, target=entryController,
            text={ str='0.00', style=4, align=0, padding=0, resizeToFit=true, col={0,204,136,255} }
          })
        end
      end
    end
  ...
  genResetBox = El:new({ parent=bodyBox, x=0, y=4, w=0, h=24, r={toEdge, bodyBox, 'right'}, flow=true, interactive=false })
  Button:new({ parent=genResetBox, x=4, y=2, style='button', target=paramsResetController, img='bin', iType=3 })
  Label:new({
    parent=genResetBox, flow=true, x=4, y=6, w=60, h=16, target=paramsResetController,
    text={ str='Reset all theme parameters to default values', style=2, align=0, padding=0,
           resizeToFit=true, col=labelColMA, mouseOverCol=labelColMO },
    onClick=function(k) k.target:onClick() end
  })
end
...
-- ===== Minimal helper shims (kept stable) =====
if type(D6) ~= 'function' then
  local D6_PREFIX = 'Default_6.0_theme_adjuster_images/'
  function D6(img) return D6_PREFIX .. img end
end

if type(TGet) ~= 'function' then
  function TGet(i)
    if type(i) ~= 'number' then return nil,nil,nil,nil,nil,nil end
    local a,b,c,d,e,f = reaper.ThemeLayout_GetParameter(i)
    return a,b,c,d,e,f
  end
end

if type(TSet) ~= 'function' then
  function TSet(i,v,commit)
    if type(i) ~= 'number' then return end
    reaper.ThemeLayout_SetParameter(i, v, commit and true or false)
  end
end
...
if type(findTcpIndex) ~= 'function' then
  local function _normD6(s)
    s = (s or ''):lower():gsub('^%s+',''):gsub('%s+$','')
    s = s:gsub('^[abc][%s_]+',''):gsub('%s*%b()',''):gsub('[^%w]+','_')
    return s
  end
  local function _findParamIndex(descKey)
    if not descKey then return nil end
    local nk = _normD6(descKey)
    for i=0,6000 do
      local ok, desc = reaper.ThemeLayout_GetParameter(i)
      if desc == nil then break end
      if _normD6(desc) == nk then return i end
    end
    return nil
  end
  function findTcpIndex(base)
    local idx = _findParamIndex(base); if idx then return idx end
    idx = _findParamIndex(base:gsub('_',' ')) or _findParamIndex(base:gsub(' ','_')); if idx then return idx end
    local b = base:gsub('^tcp[_%s]+','')
    return _findParamIndex('tcp '..b) or _findParamIndex('tcp_'..b) or _findParamIndex(b)
  end
end

-- Shim addNeedArrange if missing
if type(addNeedArrange) ~= 'function' then
  function addNeedArrange() if _G.doArrange ~= nil then _G.doArrange = true end end
end

-- ===================== D6 subpage state (separate) ===========================
-- Important: DO NOT touch global activePage here.
local D6_SUBPAGE_KEY = 'reARK_Default6_subpage'
local d6SubTag = reaper.GetExtState(sTitle, D6_SUBPAGE_KEY)
if d6SubTag == '' or d6SubTag == nil then d6SubTag = 'D6_TCP' end
_G.d6SubTag = d6SubTag

local function _asD6Tag(nameOrTag)
  return (tostring(nameOrTag):match('^D6_') and nameOrTag) or ('D6_'..tostring(nameOrTag))
end

local d6NavButtons = {}

local function setD6Sub(nameOrTag)
  local tag = _asD6Tag(nameOrTag)
  local oldTag = d6SubTag
  d6SubTag = tag
  reaper.SetExtState(sTitle, D6_SUBPAGE_KEY, d6SubTag, true)
  _G.d6SubTag = d6SubTag

  -- Clear draw coords of old/new pages so arrange() fully re-computes
  if els and oldTag ~= tag then
    for b in ipairs(els) do
      if els[b] then
        for z in ipairs(els[b]) do
          if els[b][z] then
            for _, k in ipairs(els[b][z]) do
              if k.belongsToPage == oldTag or k.belongsToPage == tag then
                k.drawX, k.drawY, k.drawW, k.drawH = nil,nil,nil,nil
                if addNeedUpdate and k.belongsToPage == tag then addNeedUpdate(k, true) end
              end
            end
          end
        end
  ...
  -- Update nav visuals
  for key, btn in pairs(d6NavButtons) do
    local btnName = key:gsub('D6_NAV_', '')
    local btnTag  = 'D6_' .. btnName
    local isActive = (btnTag == tag)
    btn.col = isActive and c_Grey15 or c_Grey20
    btn.isDirty = true
    if btn.children and btn.children[1] and btn.children[1].text then
      btn.children[1].text.col = isActive and c_Grey80 or c_CyanGrey
      btn.children[1].isDirty = true
    end
  end
...
-- ===================== Default 6.0 root (top-level) — RESPONSIVE WIDTH =======================
local default6Box = El:new({
  parent = bodyBox, x = 0, y = 0, border = boxBorder, w = 0, h = 800,
  col = c_Grey20, flow = false,
  belongsToPage = 'Default6',
  id = 'Default6Root',
  onArrange = function(k)
    local pw = (k.parent.drawW and scaleMult and k.parent.drawW/scaleMult) or k.parent.w or 980
    k.w = math.max(540, pw)
  end
})

-- ====================== Content host + subpage boxes (responsive fill) =======================
local contentHost = El:new({
  parent = default6Box, x = 0, y = 42, w = 0, h = 0,
  col = c_None, flow = false,
  onArrange = function(k)
    k.w = k.parent.w
    k.h = k.parent.h - 42
  end
...
local subPages = {'GLOBAL','TCP','MCP','COLORS','ENVELOPES','TRANSPORT'}

local D6_Boxes = {}
local function makeSubBox(tag)
  return El:new({
    parent = contentHost, x = 0, y = 0, w = 0, h = 0,
    col = c_None, flow = false,
    belongsToPage = tag,
    id = 'Box_' .. tag,
    onArrange = function(k)
...
for _, name in ipairs(subPages) do
  D6_Boxes['D6_' .. name] = makeSubBox('D6_' .. name)
end

-- ====================== GLOBAL PAGE (stub) ===================================
do
  local box = D6_Boxes['D6_GLOBAL']
  local _bp = belongsToPage; belongsToPage = 'D6_GLOBAL'
  Label:new({ parent = box, x = 250, y = 76, w = 200, h = 20,
    text = { str='GLOBAL page content goes here.', style=3, align=5, col=c_CyanGrey } })
  belongsToPage = _bp
end

-- ====================== TCP PAGE (responsive) ================================================
do
  local box = D6_Boxes['D6_TCP']
  local _bp = belongsToPage; belongsToPage = 'D6_TCP'

  -- Title + divider — responsive width
  Label:new({ parent = box, x = 15, y = 10, w = 0, h = 20,
    text = { str='Track Control Panel', style=3, align=5, col=c_CyanGrey },
    onArrange = function(k) k.w = k.parent.w - 30 end })
  El:new({ parent = box, x = 15, y = 35, w = 0, h = 2, col = c_Grey27,
    onArrange = function(k) k.w = k.parent.w - 30 end })

  -- Top control section
  local topStrip = El:new({ parent = box, x = 15, y = 50, w = 0, h = 60, col = c_Grey27,
    onArrange = function(k) k.w = k.parent.w - 30 end })

  -- Spinner helper (returns the sub-box so we can anchor it responsively)
  local function createD6Spinner(parent, x, y, baseKey, labelText)
    local paramIndex = findTcpIndex(baseKey)
    if not paramIndex then return nil end
    local sbox = El:new({ parent=parent, x=x, y=y, w=185, h=24, col=c_None })
    local controller = Controller:new({ parent=sbox, x=0, y=0, w=0, h=0, param=paramIndex, desc=labelText or baseKey })
    SpinnerH:new({ parent=sbox, x=0, y=0, z=2, h=20, w=36, target=controller })
    Readout:new({ parent=sbox, x=42, y=0, z=2, w=50, h=20, target=controller,
      text={ str='0', style=4, align=0, col={0,204,136,255}, padding=0, resizeToFit=true } })
    Label:new({ parent=sbox, x=100, y=2, w=85, h=18,
      text={ str=labelText or baseKey, style=2, align=0, col=c_CyanGrey } })
    return sbox, controller
  end

  -- Three responsive columns for the top spinners
  local s1 = createD6Spinner(box, 25, 65, 'tcp_indent',         'FOLDER INDENT')
  local s2 = createD6Spinner(box, 235,65, 'tcp_control_align',  'ALIGN CONTROLS')
  local s3 = createD6Spinner(box, 445,65, 'tcp_track_spacing',  'TRACK SPACING')
  local function _layout3(parentW, idx, total, leftPad, rightPad, gap)
    leftPad  = leftPad or 25; rightPad = rightPad or 25; gap = gap or 20
    local inner = parentW - (leftPad + rightPad)
    local w = math.floor((inner - (total-1)*gap) / total)
    local x = leftPad + (idx-1)*(w+gap)
    return x, w
  end
  if s1 then s1.onArrange = function(k) local x,w=_layout3(k.parent.w,1,3,25,25,20); k.x=x; k.w=w end end
  if s2 then s2.onArrange = function(k) local x,w=_layout3(k.parent.w,2,3,25,25,20); k.x=x; k.w=w end end
  if s3 then s3.onArrange = function(k) local x,w=_layout3(k.parent.w,3,3,25,25,20); k.x=x; k.w=w end end

  -- Layout selection controller (persist last chosen A/B/C)
  local layoutController = Controller:new({
    parent = box, x = 0, y = 0, w = 0, h = 0,
    onNewValue = function(k)
      if not k.paramV or k.paramV == 0 then
        local v = reaper.GetExtState(sTitle or 'Default_6.0 theme adjuster','activeLayoutTcp') or 'A'
        k.paramV = v
      end
      if reaper.SetExtState then
        reaper.SetExtState(sTitle or 'Default_6.0 theme adjuster', 'activeLayoutTcp', tostring(k.paramV), true)
      end
    end
  })

  -- Layout selection section — responsive
  El:new({ parent = box, x = 15, y = 125, w = 0, h = 110, col = c_Grey27,
    onArrange = function(k) k.w = k.parent.w - 30 end })
  Label:new({ parent = box, x = 25, y = 135, w = 200, h = 20,
    text = { str='LAYOUT SELECTION:', style=2, align=0, col=c_CyanGrey } })

  -- Layout buttons A/B/C (left-aligned block)
  local lx = 180
  for _, n in ipairs({'A','B','C'}) do
    Button:new({
      parent = box, x = lx, y = 145, w = 80, h = 74, style = 'button',
      target = layoutController, value = n,
      img = 'layout'..n..'_off', imgOn = 'layout'..n..'_on', iType = 3,
      onClick = function(k) k.target.paramV = n; if addNeedUpdate then addNeedUpdate(k.target) end end
    })
    lx = lx + 90
  end

  -- Apply-to-selected section — right aligned
  Label:new({ parent = box, x = 450, y = 135, w = 180, h = 20,
    text = { str='APPLY TO SELECTED TRACKS:', style=2, align=0, col=c_CyanGrey },
    onArrange = function(k) k.x = k.parent.w - 25 - k.w end })

  local function applyTcpToSelected(prefix)
    local key = 'P_TCP_LAYOUT'
    local letter = layoutController.paramV or 'A'
    local pref = prefix or ''
    if not reaper.CountTracks then return end
    for i = 0, reaper.CountTracks(0)-1 do
      local tr = reaper.GetTrack(0, i)
      if reaper.IsTrackSelected(tr) then
        reaper.GetSetMediaTrackInfo_String(tr, key, pref..letter, true)
      end
    end
  end

  local btnW, gap, rightPad = 60, 5, 25
  local function _applyBtnX(parentW, idx)
    return parentW - rightPad - ( (3-idx)*(btnW+gap) + btnW )
  end

  Button:new({ parent = box, x = 0, y = 155, w = btnW, h = 28, img = D6('apply_100'),
    onArrange = function(k) k.x = _applyBtnX(k.parent.w, 1) end,
    onClick=function() applyTcpToSelected('') end })
  Button:new({ parent = box, x = 0, y = 155, w = btnW, h = 28, img = D6('apply_150'),
    onArrange = function(k) k.x = _applyBtnX(k.parent.w, 2) end,
    onClick=function() applyTcpToSelected('150%_') end })
  Button:new({ parent = box, x = 0, y = 155, w = btnW, h = 28, img = D6('apply_200'),
    onArrange = function(k) k.x = _applyBtnX(k.parent.w, 3) end,
    onClick=function() applyTcpToSelected('200%_') end })

  Button:new({ parent = box, x = 0, y = 188, w = 180, h = 22, img = D6('apply_to_sel'),
    onArrange = function(k) k.x = k.parent.w - rightPad - k.w end,
    onClick=function() applyTcpToSelected('') end })

  layoutController.paramV = reaper.GetExtState(sTitle or 'Default_6.0 theme adjuster','activeLayoutTcp') or 'A'
  if addNeedUpdate then addNeedUpdate(layoutController) end

  -- Main control panel — responsive
  El:new({ parent = box, x = 15, y = 250, w = 0, h = 520, col = c_Grey27,
    onArrange = function(k) k.w = k.parent.w - 30 end })

  -- Control size section title
  Label:new({ parent = box, x = 25, y = 265, w = 200, h = 20,
    text = { str='CONTROL SIZES:', style=2, align=0, col=c_CyanGrey } })

  -- Reuse top 3-col layout for these size spinners
  local function _mkSizeSpinner(px, py, key, txt, slot)
    local s = createD6Spinner(box, px, py, key, txt)
    if s then s.onArrange = function(k) local x,w=_layout3(k.parent.w,slot,3,25,25,20); k.x=x; k.w=w end end
  end
  _mkSizeSpinner( 25,295,'tcp_Label',     'NAME SIZE',     1)
  _mkSizeSpinner(235,295,'tcp_Volume',    'VOLUME SIZE',   2)
  _mkSizeSpinner(445,295,'tcp_MeterLoc',  'METER LOCATION',3)

  _mkSizeSpinner( 25,325,'tcp_MeterSize', 'METER SIZE',    1)
  _mkSizeSpinner(235,325,'tcp_InputSize', 'INPUT SIZE',    2)
  _mkSizeSpinner(445,325,'tcp_SendList',  'SENDS LIST',    3)

  -- Visibility grid header (resizes)
  El:new({ parent = box, x = 25, y = 390, w = 0, h = 28, col = {36,36,36},
    onArrange = function(k) k.w = k.parent.w - 50 end })

  Label:new({ parent = box, x = 35, y = 395, w = 140, h = 18,
    text={ str='ELEMENT', style=2, align=0, col=c_CyanGrey }})

  -- Helper: scale an X based on old 595px inner width (from x=25 to x=25+595)
  local function _scaleX(parent, baseX)
    local inner = (parent.w or 650) - 50 -- 25 left + 25 right margin
    local scale = inner / 595
    return 25 + math.floor((baseX - 25) * scale)
  end

  local _hdrX  = {185, 285, 385, 485}
  local _hdrTxt = {'IF MIXER VISIBLE','IF NOT SELECTED','IF NOT ARMED','ALWAYS HIDE'}
  for i, bx in ipairs(_hdrX) do
    Label:new({
      parent = box, x = bx, y = 395, w = 100, h = 18,  -- was 120
      text={ str=_hdrTxt[i], style=2, align=5, col=(i==4 and {204,69,114} or c_CyanGrey) },
      onArrange = function(k) k.x = _scaleX(k.parent, bx) end
    })
  end

  -- Vertical separators (x scales with width)
  local _sepX = {175, 275, 375, 475}
  for _, bx in ipairs(_sepX) do
    El:new({
      parent = box, x = bx, y = 420, w = 1, h = 300, col = {64,64,64},
      onArrange = function(k) k.x = _scaleX(k.parent, bx) end
    })
  end

  -- Grid rows (responsive width + proportional columns)
  local function createGridRow(parent, base, label, icon, y)
    local shade = ((math.floor(y/28)%2)<1) and 44 or 40
    El:new({ parent=parent, x=25, y=y, w=0, h=28, col={shade,shade,shade},
      onArrange = function(k) k.w = k.parent.w - 50 end
    })

    if icon then El:new({ parent=parent, x=35, y=y+2, w=24, h=24, img=icon }) end
    Label:new({ parent=parent, x=65, y=y+5, w=110, h=18, text={ str=label, style=2, align=0, col=c_CyanGrey } })

    local flagBase = base:gsub(' ','_')
    local paramIndex = findTcpIndex(flagBase)
    local controller = nil
    if paramIndex then
      controller = Controller:new({ parent=parent, x=0, y=0, w=0, h=0, param=paramIndex, desc=label })
    end

    local function _colX(parent, base)
      local inner = (parent.w or 650) - 50
      local scale = inner / 595
      return 25 + math.floor((base - 25) * scale)
    end
    local colBase = {185, 285, 385, 485}

    for col = 1, 4 do
      local cellController = Controller:new({
        parent=parent, x=0, y=0, w=0, h=0,
        onNewValue = function(k)
          k.paramFound = false
          if controller then
            local _,_,value = TGet(paramIndex)
            value = tonumber(value) or 0
            local flag = ({1,2,4,8})[col]
            k.paramV = (value & flag) ~= 0
            k.paramFound = true
          else
            ...
            local hint = ({'if mixer is visible','if track not selected','if track not armed','always hide'})[col]
            local target = norm(base..' '..hint)
            local variations = {
              target,
              norm(base:gsub('labels values', 'labelsvalues')..' '..hint),
              norm(base:gsub('labels & values', 'labelsvalues')..' '..hint),
              norm(base:gsub('labels & values', 'labels_values')..' '..hint),
              norm(base:gsub('%s+', '_')..' '..hint)
            }
            for i=0,6000 do
              local ok,desc = reaper.ThemeLayout_GetParameter(i)
              if desc==nil then break end
              ...
              for _, v in ipairs(variations) do
                if nd:find(v,1,true) or v:find(nd,1,true) then
                  local _,_,val=TGet(i)
                  k.paramV = (tonumber(val) or 0)~=0
                  k.param = i
                  k.paramFound = true
                  break
                end
              end
              if k.param then break end
            end
          end
          if not k.isHovering and k.updateColors then k:updateColors() end
        end
      })

      cellController.updateColors = function(k)
        if k.labelRef then
          if not k.paramFound then
            k.labelRef.text.col = {255, 80, 80}
          elseif k.paramV then
            k.labelRef.text.col = c_GreenLit
          else
            k.labelRef.text.col = {80,80,80}
          end
          k.labelRef.isDirty = true
        end
      end

    local hideLabel = Label:new({
      parent = parent,
      x = _colX(parent, colBase[col]) + 25,  -- initial
      y = y+5, w = 50, h = 18,
      target = cellController, interactive = true,
      text = { str = 'HIDE', style = 2, align = 5, col = {80,80,80} },
        onMouseOver = function(k)
          if k.target and k.target.onMouseOver then k.target:onMouseOver() end
          k.target.isHovering = true
          local hoverCol = {120,120,120}
          if not k.target.paramFound then
            hoverCol = {255,120,120}
          elseif k.target.paramV then
            hoverCol = {0,255,136,255}
          end
          k.text.col = hoverCol
          k.isDirty = true
          if k.parent then k.parent.isDirty = true end
        end,
        onMouseAway = function(k)
          if k.target and k.target.onMouseAway then k.target:onMouseAway() end
          k.target.isHovering = false
          if k.target.updateColors then k.target:updateColors() end
          if k.parent then k.parent.isDirty = true end
        end,
        onClick = function(k)
          if controller and paramIndex then
            local _,_,value = TGet(paramIndex)
            value = tonumber(value) or 0
            local flag = ({1,2,4,8})[col]
            TSet(paramIndex, _bxor(value, flag), true)
          elseif k.target.param then
            local _,_,v = TGet(k.target.param)
            v = tonumber(v) or 0
            TSet(k.target.param, (v==0) and 1 or 0, true)
          end
          reaper.ThemeLayout_RefreshAll()
          addNeedArrange()
          if addNeedUpdate then addNeedUpdate(k.target) end
        end,
      onArrange = function(k)
        -- follow column scaling on resize
        k.x = _colX(k.parent, colBase[col]) + 25
      end
    })
      

      cellController.labelRef = hideLabel
      cellController.isHovering = false
      if addNeedUpdate then addNeedUpdate(cellController) end
    end
  end

  local rows = {
    {'tcp record arm','RECORD ARM', D6('key_recarm')},
    {'tcp monitor','MONITOR', D6('key_mon')},
    {'tcp track name','TRACK NAME', nil},
    {'tcp volume','VOLUME', D6('key_vol')},
    {'tcp routing','ROUTING', D6('key_route')},
    {'tcp effects','INSERT FX', D6('key_fx')},
    {'tcp envelope','ENVELOPE', D6('key_env')},
    {'tcp pan width','PAN & WIDTH', D6('key_pan')},
    {'tcp record mode','RECORD MODE', D6('key_recmode')},
    {'tcp input','INPUT', D6('key_recinput')},
    {'tcp labels values','LABELS & VALUES', nil},
    {'tcp meter values','METER VALUES', nil},
  }

  local y = 425
  for _, row in ipairs(rows) do
    createGridRow(box, row[1], row[2], row[3], y)
    y = y + 28
  end
...
-- ====================== MCP PAGE (stub) ======================================
do
  local box = D6_Boxes['D6_MCP']
  local _bp = belongsToPage; belongsToPage = 'D6_MCP'
  Label:new({ parent = box, x = 250, y = 76, w = 200, h = 20,
    text = { str='MCP page content goes here.', style=3, align=5, col=c_CyanGrey } })
  belongsToPage = _bp
end

-- ====================== COLORS PAGE (stub) ===================================
do
  local box = D6_Boxes['D6_COLORS']
  local _bp = belongsToPage; belongsToPage = 'D6_COLORS'
  Label:new({ parent = box, x = 250, y = 76, w = 200, h = 20,
    text = { str='COLORS page content goes here.', style=3, align=5, col=c_CyanGrey } })
  belongsToPage = _bp
end

-- ====================== ENVELOPES PAGE (stub) ================================
do
  local box = D6_Boxes['D6_ENVELOPES']
  local _bp = belongsToPage; belongsToPage = 'D6_ENVELOPES'
  Label:new({ parent = box, x = 250, y = 76, w = 200, h = 20,
    text = { str='ENVELOPES page content goes here.', style=3, align=5, col=c_CyanGrey } })
  belongsToPage = _bp
end

-- ====================== TRANSPORT PAGE (stub) ================================
do
  local box = D6_Boxes['D6_TRANSPORT']
  local _bp = belongsToPage; belongsToPage = 'D6_TRANSPORT'
  Label:new({ parent = box, x = 250, y = 76, w = 200, h = 20,
    text = { str='TRANSPORT page content goes here.', style=3, align=5, col=c_CyanGrey } })
  belongsToPage = _bp
end

-- ======================= Subpage navigation bar (built last; on top) — RESPONSIVE ============
local sub = El:new({
  parent = default6Box, x = 15, y = 10, w = 0, h = 28,
  flow = false, col = c_Grey25, belongsToPage = 'Default6', id='D6_SubNav',
  onArrange = function(k)
    k.w = math.max(300, k.parent.w - 30) -- margin 15 on each side
    local n   = #subPages
    local gap = 8
    local left = 10
    local inner = k.w
    local btnW = math.floor((inner - left*2 - gap*(n-1)) / n)
    local bx = left
    for _, name in ipairs(subPages) do
      local btn = d6NavButtons['D6_NAV_'..name]
      if btn then
        btn.x = bx
        btn.w = btnW
        -- Expand label + hit target
        if btn.children then
          for _, ch in ipairs(btn.children) do
            if ch.text then ch.x = 0; ch.w = btnW end
            if ch.onClick then ch.x = 0; ch.w = btnW; ch.h = btn.h end
          end
        end
      end
      bx = bx + btnW + gap
...
do
  ...
  for _, name in ipairs(subPages) do
    local tag = 'D6_' .. name
    local isActive = (tag == d6SubTag)
    local btnKey = 'D6_NAV_' .. name

    local btnContainer = El:new({
      parent = sub, x = x, y = 2, w = 85, h = 24,
      col = isActive and c_Grey15 or c_Grey20,  -- Darker when active
      belongsToPage = 'Default6',
      id = btnKey
    })
    d6NavButtons[btnKey] = btnContainer

    Label:new({
      parent = btnContainer, x = 0, y = 4, w = 85, h = 16, interactive = false,
      text = { str = name, style = 2, align = 5, col = isActive and c_Grey80 or c_CyanGrey },
      belongsToPage = 'Default6',
      onArrange = function(k) k.w = k.parent.w end
    })

    do
      local thisTag = tag
      Button:new({
        parent = btnContainer, x = 0, y = 0, w = 85, h = 24,
        mouseOverCol = {255,255,255,10},
        belongsToPage = 'Default6',
        onClick = function()
          setD6Sub(thisTag)
          return true
